...
class HBaseRegionserver(HadoopHttp):
    ...
    def emit_region_metric(self, context, current_time, full_metric_name, value, step):
        ...
        if any(not v for k, v in tag_dict.iteritems()):
            ...
            self.emit_metric(context, current_time, metric_name, value, step, tag_dict)
    ...
    def emit(self):
        ...
        for context, metric_name, value in metrics:
            ...
            if any(c == "regions" for c in context):
                if EMIT_REGION:
                    self.emit_region_metric(context, current_time, metric_name, value, step)
            else:
